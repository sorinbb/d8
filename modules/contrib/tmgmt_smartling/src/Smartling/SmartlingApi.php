...
class SmartlingApi
{
    ...
    protected function sendRequest($uri, $requestData, $method, $filenames = [])
    {
        ...
        if (in_array($method, ['GET', 'DELETE'])) {
            ...
            foreach ($requestData as $key => $value) {
                ...
                if (isset($filenames[$key])) {
                    ...
                    $val['Content-Type'] = $filenames[$key]['content_type'];//mime_content_type($val['filename']);
                }
        ...
        if ($guzzle_response->getStatusCode() >= 400) {
            ...
            throw new SmartlingApiException(print_r($error_response['response']['errors'], TRUE), $guzzle_response->getStatusCode());
        }
    ...
    public function uploadContext($data, $filenames)
    {
        ...
        return $this->sendRequest('js/' . $orgID . '/html', $data, 'POST', $filenames);
    }
    ...
    public function putResource($data, $filenames) {
        ...
        return $this->sendRequest('js/projects/' . $orgID . '/resources/' . $data['resourceId'], $data, 'PUT', $filenames);
    }
